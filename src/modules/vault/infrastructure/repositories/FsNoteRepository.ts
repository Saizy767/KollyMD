...
export class FsNoteRepository implements NoteRepository {
  ...
  listFolders(vaultRoot: string): FolderDto[] {
    const folders: FolderDto[] = []
    this.collectFolders(vaultRoot, folders)
    return folders
  }
  ...
  private collectFolders(dirPath: string, folders: FolderDto[]): void {
    let names: string[]
    try {
      names = fs.readdirSync(dirPath)
    } catch (e) {
      this.logger.warn('Failed to read directory during folder collection', { dirPath, error: (e as Error).message })
      return
    }
    ...
    for (const name of names) {
      ...
      if (isDir) {
        folders.push({ path: fullPath, name })
        this.collectFolders(fullPath, folders)
      }
    }
  }
